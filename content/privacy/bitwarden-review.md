...
**沒有 TOTP 整合**：免費版不能在 Bitwarden 裡儲存和產生驗證碼（如 Google 兩步驟驗證的六位數字），需要另外用 Google Authenticator 或類似的 App。這是免費版最常被提到的不足。如果 TOTP 整合對你很重要，可以參考 [Bitwarden vs 1Password 的功能比較](/privacy/bitwarden-vs-1password/)，看看升級是否值得。
