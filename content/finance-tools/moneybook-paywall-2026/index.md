...
date: 2026-04-15
lastmod: 2026-09-15
description: "麻布記帳 2026 年 4 月起全面收費：現在月費 NT$149、年費 NT$690（只到 2026 年底），2027 年起年費 NT$1,499。評測付費後買到什麼、缺點在哪，並比較 CWMoney、MOZE、Notion 三個替代方案，直接告訴你該留還是換。"
...
> 📅 原文發布：2026 年 4 月｜最後更新：2026 年 9 月
> 本文價格、試用與替代方案已於 2026 年 9 月對照麻布記帳[官方收費公告](https://blog.moneybook.com.tw/?p=6341)、[官網](https://moneybook.com.tw/)、App Store 訂閱方案與各替代 App 官方頁面重新校對。
...
<figcaption style="font-size:13px;color:#5A6B82;margin-top:8px;">資料來源：麻布記帳 2026/03/13 官方公告。年費 NT$690 維持到 2026/12/31，2027 年起年費 NT$1,499（+117%）；想用 NT$690 年費，決策窗口在 2026/12/31 之前。</figcaption>
</figure>
...
麻布記帳的核心賣點一直是**自動銀行同步**：帳戶餘額、消費明細自動匯入，不用手動逐筆輸入。這也是它跟 [MOZE](https://moze.app/)、[CWMoney](https://www.cwmoney.net/) 最大的差異點。
...
麻布記帳目前主要走「代理同步」：你提供網銀登入資訊，由它代你登入銀行網頁抓資料。[金融監督管理委員會](https://www.fsc.gov.tw/)推動的「[開放銀行（Open Banking）](https://en.wikipedia.org/wiki/Open_banking) API」還在逐步推進，官方說明在 API 全面普及前，代理同步會和 API 長期並行（[官方說明](https://blog.moneybook.com.tw/?p=1105)）。官方有 [ISO/IEC 27001](https://en.wikipedia.org/wiki/ISO/IEC_27001) 與 ISO 27701 資安認證，但「把網銀帳密交給第三方 App」這件事，對一部分人就是過不去的門檻——下一段把這個疑慮拆開講。
...
台灣的開放銀行（Open Banking）目前停在第二階段（客戶資料），多數銀行還沒開放標準化的 OAuth 授權，所以麻布對許多銀行仍走「代理登入」——也就是你得把網銀帳密交給它，由它代你登入抓資料。這跟「銀行直接授權第三方讀取」是兩回事：認證再多，只要是代理帳密模式，帳密就多經過一手。對這點在意的人，這不是被認證能完全消除的疑慮，而是一個你要自己決定能不能接受的取捨。代理登入跟 OAuth 授權實際差在哪、哪些風險是真的、哪些是誤會，另一篇[麻布記帳自動抓帳真的安全嗎？代理同步 vs 開放銀行 API 資安評測](/finance-tools/moneybook-security/)拆得更完整。
